...
monetochka = ('Орёл', 'Решка')

@app.route("/")
def hello():
    return 'Привет, по ссылке ниже ты найдёшь рандомный факт про технологическую зависимость!, <a href="/random_fact">Посмотреть случайный факт!</a>'
...
@app.route("/monetka")
def monetka():
    return '<h1>Бросок монетки!</h1>' f'<p>{random.choice(monetochka)}</p>'
